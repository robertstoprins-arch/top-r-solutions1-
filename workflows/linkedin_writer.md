...
- **The model fabricates specifics to sound impressive.** Observed live (2026-09-08): given a short, factual topic ("same team, demolition to handover, on time and on budget"), the model invented a "90% of projects suffer mid-project team changes" statistic, a nonexistent "internal tracking system," and a fabricated "15-minute conversation that prevents a 3-month delay" outcome — none of it present in the input, all of it stated as fact. It also wrapped phrases in `**bold**`/`*italic*` markdown, which LinkedIn renders as literal asterisks (an instant AI tell). Fixed by adding explicit anti-fabrication and no-markdown rules to `buildStyleRulesBlock()` and `topicLockBlock()` in `api/_voice.js` (and separately to `buildCommentSystem`, which doesn't share that block): the topic/key-points text is the *only* permitted source of facts, and the model is told to write shorter rather than invent detail when the input is thin. Also added rules against generic AI tells (the "it's not X, it's Y" contrast cliché, mechanical rule-of-three, and the repeated "Beyond X, what's your Y?" CTA shape). If fabrication reappears, check whether it's the drafting pass (Pass 1) or the rewrite pass (Pass 3, which sees the critique and can over-elaborate to satisfy it) and reinforce whichever builder is generating it.
