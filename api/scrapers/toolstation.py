...
Search URL: https://www.toolstation.com/search?q={query}

SELECTOR CONSTANTS — update these if Toolstation changes their DOM.
...
# ── Selectors ─────────────────────────────────────────────────────────────────
PRODUCT_CARD  = '.product, [class*="product-item"], [class*="ProductItem"], article[class*="product"]'
PRODUCT_TITLE = 'h3, h2, [class*="product-title"], [class*="ProductTitle"], [class*="name"]'
PRICE_MAIN    = '[class*="price"], [class*="Price"], [data-price]'
AVAILABILITY  = '[class*="availability"], [class*="stock"], [class*="Stock"]'
PRODUCT_LINK  = 'a[href]'


class ToolstationScraper(BaseSupplierScraper):
    ...
    async def get_price(self, page, item: str, quantity: float, unit: str, postcode: str) -> PriceResult:
        ...
        try:
            ...
            try:
                consent = await page.query_selector('[id*="cookie"] button, #cookieAccept, [class*="cookie-accept"]')
                if consent:
                    await consent.click()
...
def _parse_availability(text: str) -> str:
    if any(w in text for w in ["in stock", "available", "add to basket"]):
        return "in stock"
...
def _parse_delivery(text: str) -> str:
    if any(w in text for w in ["next day", "next working day", "tomorrow"]):
        return "next day"
